...
from model.layers.blocks import (
    Attention,
...
from util.registry import MODELS
from util.ckpt_utils import auto_grad_checkpoint,load_checkpoint
...
class STDiT2Block(nn.Module):
    def __init__(
        ...
        self.attn_temp = Attention(
            ...
            rope=rope,       # self.rope = RotaryEmbedding(dim=self.hidden_size // self.num_heads)  # new
            qk_norm=qk_norm,
            qk_norm_legacy=qk_norm_legacy,
        )
...
class STDiT2(PreTrainedModel):
    ...
    def forward(
        ...
        _, _, Tx, Hx, Wx = x.size()
        T, H, W = self.get_dynamic_size(x)  # 每个维度上有多少个patch
        S = H * W                           # 空间维度上有多少个patch
